/**
 * @file cstats.c
 * @brief Implementação da biblioteca CStatisticsLibrary
 */
...
/**
 * @brief Função de comparação para qsort (ordenação crescente)
 */
                     ...
                     double *modes, size_t max_modes)
{
    ...
    /* Primeiro passo: determinar a frequência máxima */
    size_t max_count = 1;
    size_t current_count = 1;
    ...
    /* Segundo passo: coletar todas as modas */
    size_t mode_index = 0;
    current_count = 1;
...
double cstats_quantile(const double *data, size_t size, double quantile)
{
    ...
    if (index_lower == index_upper) {
        ...
        result = sorted[index_lower] 
               + fraction * (sorted[index_upper] - sorted[index_lower]);
    }
                                  ...
                                  double *residual_variance)
{
    ...
    if (r_squared) {
        *r_squared = (ss_total == 0.0) ? NAN : 1.0 - ss_residual / ss_total;
    }
    if (residual_variance) {
        *residual_variance = ss_residual / (double)(size - 2);
    }
